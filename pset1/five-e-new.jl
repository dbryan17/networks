...
function get_all_edges(names) :: Dict{String, Set{Tuple{Int, Int}}}
  ret = Dict()
...
names = ["Berkeley13.txt", "Bingham82.txt", "GWU54.txt", "Haverford76.txt", "Johns Hopkins55.txt"]
...
edges :: Dict{String, Set{Tuple{Int, Int}}} = get_all_edges(names)
...
function apsp(adj_list :: Dict{Int, Set{Int}}, nodes :: Vector{Int}, name_count :: Int)
...
for (name, edges) in edges
  ...
  for v in vertices[name]
    if !get(visited, v, false)
        component = []
        dfs_iterative(v, ad_list, visited, component)
  ...
  all_pairs_distances = apsp(new_adj_list, largest_comp, name_count)
  ...
  open("data/" * string(name) * ".txt", "w") do file
    println(file, name)
    println(file, "average dist")
    println(file, string(average_dist))
    println(file, "size of largest comp")
    println(file, string(size_of_largest_comp))
    println(file, "max dist")
    println(file, string(max_dist))
    println(file, "total nodes")
    println(file, length(vertices[name]))
  end
...
avg_fig = scatter(avg_fig_x, avg_fig_y, title="Mean geodesic distance vs largest component size", xlabel="largest component size", ylabel="mean geodesic distance", color=:blue, markersize=5)
